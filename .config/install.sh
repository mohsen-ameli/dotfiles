#!/bin/bash
# Installs the packages that I use on my Arch Linux setup

curr_dir=$(dirname "$0")
emulator="kitty"
shell="zsh"
editor="code nano vim nvim gedit"
file_explorer="dolphin lc tumbler ffmpegthumbnailer"
image_viewer="qimgv"
...
network="networkmanager iwd net-tools"
bluetooth="bluez bluez-utils bluez-obex bluetuith"
...
setup_packages() {
    ...
    $pkg_manager --noconfirm -Syyu base-devel git $network $themes $shell $file_explorer $image_viewer \
        $media_player $emulator $editor $fonts $bluetooth $audio $app_launcher $rofi $display_manager \
        zenity pacman-contrib ffmpeg jq grim slurp brightnessctl ntfs-3g socat inotify-tools discord \
        zip dunst htop nwg-look koi localsend \
    }

setup_hyprland() {
    confirm "Do you want to set up Hyprland as your window manager?" || return
    notify ":: Setting up Hyprland"
    $pkg_manager --noconfirm -S hyprland hyprpicker hyprutils hyprwayland-scanner python-pywal eww swww swaybg hyprlock hypridle cliphist xdg-desktop-portal-hyprland-git
    sudo systemctl enable --now sddm
    mkdir -p $HOME/.config/hypr
    cp $HOME/dotfiles/hypr/hyprland.conf $HOME/.config/hypr/hyprland.conf
    mkdir -p $HOME/.config/eww
    cp -r $HOME/dotfiles/eww/* $HOME/.config/eww/
    mkdir -p $HOME/.config/swaybg
    cp $HOME/dotfiles/swaybg/config $HOME/.config/swaybg/config
    mkdir -p $HOME/.config/swww
    cp $HOME/dotfiles/swww/swww.conf $HOME/.config/swww/swww.conf
    mkdir -p $HOME/.config/cliphist
    cp $HOME/dotfiles/cliphist/config.yml $HOME/.config/cliphist/config.yml
}

setup_g14() {
    confirm "Do you have an ASUS ROG G14 laptop?" || return
    notify ":: Setting up ASUS ROG G14 specific packages"
...
[g14]
    ...
    $pkg_manager -Syyu
    $pkg_manager -S asusctl rog-control-center supergfxctl
}

setup_kde() {
    confirm "Do you want to install KDE Plasma Desktop Environment?" || return
    notify ":: Installing KDE Plasma"
    $pkg_manager -S plasma-desktop
}
...
setup_nvidia() {
    confirm "Do you have an NVIDIA GPU?" || return
    notify ":: Setting up NVIDIA GPU"
    $pkg_manager -S nvidia-open nvidia-settings nvidia-utils lib32-nvidia-utils
}
...
setup_samba() {
    confirm "Do you want to enable SAMBA file sharing?" || return
...
setup_browser() {
    ...
    printf "Which browser do you want to install: (1) Chrome (2) Firefox (3) Brave (4) Librewolf"
    read choice
    if [ $choice -eq 1 ]; then
...
setup_other() {
    confirm "Do you want to set default applications and other configurations?" || return
    notify ":: Setting default applications and other configurations"
